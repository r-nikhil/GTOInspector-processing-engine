...
class Parser():
    ...
    def __init__(self, **kwargs):
        # Classes
        # self.helper = kwargs.get("helper", Helper())
        self.helper = Helper()
        # self.helper = None
    ...
    def get_action_chain(self, list_of_lines, num_players):
        ...
        for line in list_of_lines:
            # TODO: Flop processing
            # if "TURN" in line:
            #     break
            if "FLOP" in line:
                break
                # flop = self.cards_regex.findall(line)
                # if len(flop) == 3:
                #     flop = self.helper.rearrange_cards_anti_alphabetically(
                #         flop)
                #     flop = "".join(flop)
                #     action_sequence.append(flop)
            else:
    ...
    def process_section(self, list_of_lines):
        ...
        for action_sequence in action_sequences:
            ...
            return_values.append({
                ...
                "Move EV": res_dict[current_action[0]]["ev"]/2000,
                "GTO EV": highest_ev
            })
...
Seat 6: nimit2908 (small blind) folded on the River"""
    ...
    # print(parser.process_section(list_of_lines))
    # print(parser.heroname)
    # parser.set_heroname(list_of_lines)
    # print(parser.heroname)
    # print(parser.get_amount_won(list_of_lines))
    # print(parser.get_action_chain(list_of_lines))
